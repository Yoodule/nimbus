...
의도된 동작입니다. 기본적으로 OAuth 토큰은 메모리에만 존재하며, 매 재시작이 새로운 재승인을 의미합니다. 공유 / 다중 사용자 호스트에서 더 안전한 기본값이기 때문입니다. 영구 토큰을 원한다면 다운로드 FAQ 의 [OAuth 영구화 섹션](download.md#where-do-my-oauth-tokens-live) 을 참고하세요.
